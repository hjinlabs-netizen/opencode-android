...
/**
 * Sprint 1c.2 — Tier A (JVM) SSE integration matrix. Drives the PRODUCTION
 * stack end-to-end over real loopback sockets: `NetworkModule`'s OkHttp
 * client, the real `KtorSseConnector`, [SseEventTransport] probe/memoization,
 * the [OpenCodeStreamClient] supervisor (backoff, status, SharedFlow) and
 * [ChatStreamRepositoryImpl]'s config-driven lifecycle. No connector fakes.
 *
 * Fixture: [SseTestServer] (sharedTest) — MockWebServer proved incompatible
 * with the Ktor OkHttp SSE engine in the 1c.1 spike.
 */
class SseEngineIntegrationTest {

    private val json = NetworkModule.provideJson()
    private val client: HttpClient = NetworkModule.provideHttpClient(json)
    ...
    private fun transportFor(connection: FakeConnectionRepository): SseEventTransport =
    ...
    private fun config(url: String, token: String = "tok") = ServerConfig(url, token)

    private fun recordStatuses(stream: OpenCodeStreamClient): CopyOnWriteArrayList<StreamStatus> =
    ...
    private suspend fun awaitUntil(what: String, deadlineMs: Long = 12_000, condition: () -> Boolean) {
    ...
    private fun errorOf(statuses: List<StreamStatus>): OpenCodeError? =
        statuses.filterIsInstance<StreamStatus.Error>().lastOrNull()?.error

    private fun SseTestServer.Connection.holdOpen() {
        try {
    ...
    private fun delta(session: String, text: String) =
    ...
    fun `burst of 500 frames against a slow subscriber arrives complete and ordered`() {
        ...
        assertEquals((0 until 500).toList(), sequence)
    }
    ...
    fun `winner memoization skips probes on reconnect and demotes a moved route`() {
        ...
        suspend fun drain(): List<String> = withTimeout(TIMEOUT_MS) { transport.frames(cfg).toList() }
